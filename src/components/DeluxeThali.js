...
import deluxe3 from './deluxe3.jpg';
...
const DeluxeThali = () => {
    ...
    // Filter backend foods with tag 'deluxe thali'
    const backendFoods = foodListData?.data?.filter(food => 
        food.tags?.some(tag => tag.toLowerCase() === 'deluxe thali')
    ).map(food => ({
        id: food._id,
        title: food.foodName,
        price: food.price,
