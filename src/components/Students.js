...
// Import images
import studentImg1 from './student1.jpg';
import studentImg2 from './student2.jpg';
...
const Students = () => {
    ...
    // Filter backend foods with tag 'students'
    const backendFoods = foodListData?.data?.filter(food => 
        food.tags?.some(tag => tag.toLowerCase().trim() === 'students')
    ).map(food => ({
        id: food._id,
        title: food.foodName,
        price: food.price,
